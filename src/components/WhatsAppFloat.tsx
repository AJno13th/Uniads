...
/**
 * Floating WhatsApp CTA. Layout is CSS-first so SSR/CSR markup matches
 * (avoids React hydration #418). On mobile the chip starts compact and
 * opens chat on the first tap.
 */
export function WhatsAppFloat() {
  return (
    <div className="fixed bottom-[max(1rem,env(safe-area-inset-bottom))] right-4 z-[60] flex items-end gap-2 sm:right-5">
      <a
        href={whatsappLink()}
        target="_blank"
        rel="noopener noreferrer"
        className="whatsapp-pulse flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg transition hover:bg-[#1ebe57] sm:h-auto sm:w-auto sm:max-w-[min(100vw-2rem,18.5rem)] sm:gap-3 sm:px-4 sm:py-3"
      ...
      >
        ...
        <span className="hidden pr-1 text-left text-xs font-semibold leading-snug sm:inline sm:text-sm">
