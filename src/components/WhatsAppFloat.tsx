...
/**
 * Compact on mobile so forms stay usable; expands on larger screens.
 * Mature students on TikTok ads mostly arrive on phones.
 */
    ...
    <div className="fixed bottom-[max(1rem,env(safe-area-inset-bottom))] right-4 z-[60] flex items-end gap-2 sm:right-5">
      {isMobile && expanded && (
        <button
          ...
          className="mb-2 rounded-md bg-navy/80 px-2 py-1 text-[11px] font-semibold text-white"
          aria-label="Collapse WhatsApp message"
        >
          Close
        </button>
      )}
      <a
        ...
        className={`whatsapp-pulse flex items-center gap-3 rounded-full bg-[#25D366] text-white shadow-lg transition hover:bg-[#1ebe57] ${
          showLabel ? "max-w-[min(100vw-2rem,18.5rem)] px-4 py-3" : "h-14 w-14 justify-center"
        }`}
